...
public class ToroQuestCommand extends CommandBase {
	...
	private void genCommand(EntityPlayer player, String[] args) throws CommandException {
		...
		if ("throne_room".equals(structure)) {
			new ThroneRoomGenerator().generate(player.getEntityWorld(), new Random(), player.getPosition());
			return;
		}

		if ("mage_tower".equals(structure)) {
			new MageTowerGenerator().generate(player.getEntityWorld(), new Random(), player.getPosition());
			return;
		}

		if ("bastions_lair".equals(structure)) {
			new BastionsLairGenerator().generate(player.getEntityWorld(), new Random(), player.getPosition());
			return;
		}

		if ("monolith".equals(structure)) {
			new MonolithGenerator().generate(player.getEntityWorld(), new Random(), player.getPosition());
			return;
		}

		if ("graveyard".equals(structure)) {
			new GraveyardGenerator().generate(player.getEntityWorld(), new Random(), player.getPosition());
			return;
		}
