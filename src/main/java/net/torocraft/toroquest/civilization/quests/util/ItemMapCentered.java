...
public class ItemMapCentered extends ItemMap
{
    ...
    public static ItemStack setupNewMap(World worldIn, double worldX, double worldZ, byte scale, boolean trackingPosition, boolean unlimitedTracking)
    {
        ...
        mapdata.scale = scale;
        calcMapCenter(mapdata, worldX, worldZ, mapdata.scale);
    ...
    {
    	int i = (1 << mapScale);
        int j = MathHelper.floor(x / (double)i);
        int k = MathHelper.floor(z / (double)i);
        mapdata.xCenter = j * i + i / 2;
        mapdata.zCenter = k * i + i / 2;
        
//      int i = 16 * (1 << mapScale);
//      int j = MathHelper.floor((x + 8.0D) / (double)i);
//      int k = MathHelper.floor((z + 8.0D) / (double)i);
//      mapdata.xCenter = j * i + i / 2 - 8;
//      mapdata.zCenter = k * i + i / 2 - 8;
    }
    ...
    {
        if (map.getItem() instanceof ItemMapCentered)
        {
            MapData mapdata = ((ItemMapCentered) map.getItem()).getMapData(map, worldIn);
