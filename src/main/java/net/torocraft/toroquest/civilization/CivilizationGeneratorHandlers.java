					...
					for ( int zz = 0; zz < 16; zz++ )
					{
						for ( int yy = CivilizationHandlers.SPAWN_HEIGHT; yy >= y; yy-- )
						{
							...
							if ( b instanceof BlockChest && ToroQuestConfiguration.replaceVillageChestsWithTrappedChests )
							{
								try
								{
									...
									if ( chest.chestType != BlockChest.Type.TRAP )
									{
										...
										{
											loot = ((TileEntityChest) tile).getLootTable();
										}
										...
										if ( loot != null && tile instanceof TileEntityChest )
										{
											((TileEntityChest) tile).setLootTable(loot, new Random().nextLong());
										}
									}
								}
								catch ( Exception e )
							...
							if ( destroyedVillage )
							{
								...
								else if ( b instanceof BlockStone || b instanceof BlockDirt )
								{
									break;
								}
								else if ( b instanceof BlockGrass || b instanceof BlockSand )
								{
									if ( hasVillagerChunk && event.getRand().nextInt(128) == 0 ) // TODO
									{
								...
								else if ( event.getWorld().rand.nextInt(7) == 0 )
								{
									...
									{
										event.getWorld().setBlockState(pos, Blocks.DIRT.getDefaultState());
									}
